...
void SCRLE::initialize(int stage)
{
...
void SCRLE::handleMessage(cMessage *msg)
{
...
void SCRLE::recievePlist(cMessage* msg)
{
    plist_msg++;
    ...
    if(getStatus()!=0)//sending CRL if we have some pieces
    {
...
int SCRLE::getStatus()
{
    int pieces_no = PiecesCRLcollected.size();
    if(pieces_no == 0)
        return 0;
    else if(pieces_no<(totalPieces/4)) //less than 25% of total pieces
        return 1;
    else if(pieces_no< (totalPieces/2)) //less than 50% of total pieces
        return 2;
...
void SCRLE::recieveBeacon(cMessage *msg)
{
    ...
    if ( getEnvir()->isGUI() )
    {
       ...
       //     nodePtr->bubble("received a broadcast beacon");
        ...
        if(selectedBeacon){
            if(selectedBeacon->getMyResult() <= m->getMyResult())
...
void SCRLE::recieveCRL(cMessage *msg)
{
...
void SCRLE::CRLreconstruction_NoErasure()
{
...
void SCRLE::Send_CRL_List_Msg(int rSerial,veins::LAddress::L2Type recId)
{
    ...
    if(rSerial == -1)
    {
        ...
        serial = serial;
    ourSerial = rSerial;//storing our Serial to know that we have recieved our own Plist message
...
void SCRLE::UpdateMask(Piece_list *m)
{
...
void SCRLE::sendCRLsMask(veins::LAddress::L2Type recID)
{
    if ( getEnvir()->isGUI() )
    {
